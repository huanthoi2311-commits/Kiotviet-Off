...
**Version hiện tại:** `v0.3.0-category-foundation` (T007 Brand đã code xong, **chưa tag** — chờ Architecture Review riêng cho tag/push, xem `docs/release/t007-release-note.md` mục "Release Review")
**Sprint hiện tại:** Sprint-01 (đang tiến hành)
**Task tiếp theo:** T007 Brand chờ Architecture Review cuối cùng để tag/push (`v0.4.0-brand-foundation` dự kiến). Sau đó bắt đầu T008 — Unit Domain (chờ RFC-0004).
...
| T007 | Brand Domain | `SPEC-BRAND-001` | 🟡 Code xong (Technical Complete), chờ Architecture Review để tag | `v0.4.0-brand-foundation` (dự kiến) |
...
Từ T006 trở đi, **mỗi Sprint task mới phải xác nhận toàn bộ Task đã DONE trước đó vẫn PASS** — chạy `npx jest` toàn bộ, không chỉ phạm vi module đang làm. Hiện tại (sau T007): **139/139 test suite PASS, 1318/1318 test PASS** — xác nhận T005 (Product) + T006 (Category) + Auth/RBAC/Inventory/Organization/Branch đều không bị ảnh hưởng bởi T007. Baseline tiếp theo (T008) sẽ gồm T005+T006+T007.
