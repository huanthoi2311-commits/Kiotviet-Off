...
/**
 * T053.05C-1 — script CLI mỏng, chỉ đọc, cùng pattern các script `prisma/*.ts` khác
...
async function main() {
  ...
  try {
    ...
    console.log(
      '=== T053.05C-1 Tenant Foreign-ID Integrity Audit (chỉ đọc) ===\n',
    );
    ...
    if (total > 0) {
      ...
      console.log(
        '\nKhông phát hiện dữ liệu tenant-inconsistent nào trong phạm vi T053.05C-1.',
      );
    }
  } finally {
