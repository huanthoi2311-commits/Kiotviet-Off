...
- ⚠️ **Chưa xác định rõ, cần Architect xác nhận lại** — `ARCHITECTURE REVIEW – T008 Unit Implementation` (Decision UR08) nêu T009 = Customer Domain, khác roadmap cố định đã thiết lập nhiều lần trước đó (Decision T007-03 và các Decision kế thừa: Brand → Unit → Barcode → Attribute → Variant, "không được bỏ qua thứ tự"). Customer cũng thuộc nhóm CRM, không phải Master Data. Xem `PROJECT_STATUS.md` mục "Ghi chú roadmap" để biết chi tiết — không tự chọn 1 trong 2 phương án.
