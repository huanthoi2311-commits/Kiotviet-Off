...
- Roadmap cố định (Decision U/RQ-series): Brand → **Unit (T008, xong)** → Barcode → Attribute → Variant. Bước tiếp theo dự kiến: T009 — Barcode Domain, chờ RFC-0005 từ Architect.
...
Theo đúng AUTHORIZATION gốc ("...→ Tests → Documentation → **Release Review**"), tài liệu này hoàn tất bước Documentation. **Tag (`v0.5.0-unit-foundation` dự kiến) và push chưa được thực hiện** — chờ Architect xác nhận qua 1 Architecture Review riêng cho toàn bộ T008 Implementation, đúng tiền lệ T007 (tag/push chỉ thực hiện sau `ARCHITECTURE REVIEW – T007 Brand Implementation` riêng biệt với Decision R06 cho phép tag/push).
