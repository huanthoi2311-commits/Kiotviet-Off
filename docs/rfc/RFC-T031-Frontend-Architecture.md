...
- **Login**: `POST /auth/login` with `organizationSlug + email + password` (multi-tenant — email is only unique per-organization). WEB clients (no `X-Client-Type: mobile` header) receive the access token in the JSON body; the refresh token is delivered **only** via an `HttpOnly`, `SameSite=Lax` cookie scoped to `Path=/` (T051.08C — widened from `/api/v1/auth`, which never matched the frontend's own protected page routes and left `middleware.ts` unable to observe the cookie at all) — never present in the WEB response body. `Secure` is configurable via the backend's `AUTH_COOKIE_SECURE` env var (T051.08B) — `false` for the packaged HTTP `localhost` V1 deployment, `true` behind HTTPS — not hardcoded to the runtime's `NODE_ENV`.
