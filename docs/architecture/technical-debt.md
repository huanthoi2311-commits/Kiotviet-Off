...
- **Phát sinh từ:** Sprint-00 (T004), tiếp tục ở T005, T006.
- **Nguyên nhân:** Sandbox phát triển hiện tại không có Docker/Postgres/Redis — không thể chạy `npm run test:e2e` thật.
- **Điều kiện để hoàn thành:** Có môi trường Docker (`docker-compose up`), chạy `npm run test:e2e`, xác nhận toàn bộ `test/*.e2e-spec.ts` PASS. Với `category`: cần tạo mới `test/category.e2e-spec.ts` (chưa tồn tại).
...
### 2. Rollback Test — migration của T005 (Product) và T006 (Category)

- **Nguyên nhân:** Cùng lý do #1 — không có Postgres để chạy `up → rollback.sql → up` thật.
- **Điều kiện để hoàn thành:** Chạy từng `rollback.sql` (7 file: 3 của T005 + 3 của T006, cộng 1 file gộp T005 Migration 1+2) trên Postgres thật, xác nhận schema về đúng trạng thái trước migration, chạy `up` lại lần 2 xác nhận idempotent.
...
### 3. Manual API Smoke Test — Product (T005) và Category (T006)

- **Nguyên nhân:** Cần app chạy thật (`npm run start:dev`, cần Postgres/Redis) để gọi HTTP endpoint qua Swagger UI/curl.
- **Điều kiện để hoàn thành:** Khởi động app thật, gọi đủ route của `product` (6 route) và `category` (7 route), xác nhận response shape khớp DTO hiện tại.
...
*(chưa có mục nào — sẽ cập nhật khi 1 trong 4 mục trên chuyển PASS thật)*
